...
def proxy(path):
    ...
    try:
        resp = requests.request(method=request.method, url=target_url, headers=headers, data=request.get_data(), params=request.args)
        ...
        key_manager.update_key_stats(key_id, resp.ok, model_name,
            error_code=None if resp.ok else resp.status_code,
